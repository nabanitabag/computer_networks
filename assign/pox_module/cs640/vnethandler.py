...
class VNetDevice:
  ...
  def handle_packet_msg(self, vns_msg):
    ...
    try:
      ...
      log.debug('packet-out through wrong interface %s' % out_intf)
      return
    log.debug("Packet out %s.%s" % (self.swid, out_intf))
  ...
  def handle_VNetPacketIn(self, event):
    try:
      ...
      log.debug("Couldn't find interface for port %s" % event.port)
      return
    log.debug("Packet in %s.%s" % (self.swid, intfname))
...
class VNetServerListener(EventMixin):
  ...
  def _handle_VNetPacketIn(self, event):
    dev = self.devsByName.get(event.swid)
    if dev is None:
      return
  ...
  def handle_open_msg(self, conn, vns_msg):
    dev = self.devsByName.get(vns_msg.vhost)
    if dev is None:
  ...
  def handle_client_disconnect(self, conn):
    ...
    self.devsByConn.pop(conn, None)
    conn.transport.loseConnection()


class VNetPacketOut(Event):
  '''Event to raise upon receiving a packet back from VNet device'''

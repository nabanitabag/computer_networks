...
class VNSOpen(LTMessage):
    ...
    def pack(self):
    # Change this:
        return struct.pack(VNSOpen.FORMAT, self.vhost.encode('ascii'))

    @staticmethod
    def unpack(body):
        t = struct.unpack(VNSOpen.FORMAT, body)
        # Decode the bytes to a string
        vhost = strip_null_chars(t[0].decode('ascii')) 
...
class VNSClose(LTMessage):
    ...
    def pack(self):
        return struct.pack(VNSClose.FORMAT, self.msg.encode('ascii'))

    @staticmethod
    def unpack(body):
        t = struct.unpack(VNSClose.FORMAT, body)
        # Decode the bytes to a string
        msg = strip_null_chars(t[0].decode('ascii'))
        return VNSClose(msg)
...
class VNSPacket(LTMessage):
    ...
    def pack(self):
    # intf_name needs encoding, ethernet_frame is usually already bytes
        name = self.intf_name.encode('ascii') if isinstance(self.intf_name, str) else self.intf_name
        return struct.pack(VNSPacket.HEADER_FORMAT, name) + self.ethernet_frame

    @staticmethod
    def unpack(body):
        t = struct.unpack(VNSPacket.HEADER_FORMAT, body[:VNSPacket.HEADER_SIZE])
        # Decode interface name to string
        intf_name = strip_null_chars(t[0].decode('ascii'))
...
class VNSInterface:
    ...
    def pack(self):
        return struct.pack(VNSInterface.FORMAT,
                       VNSInterface.HWINTERFACE, self.name.encode('ascii'),
                       VNSInterface.HWETHER, self.mac,
                       VNSInterface.HWETHIP, self.ip, b'', # Change '' to b''
                       VNSInterface.HWMASK, self.mask, b'') # Change '' to b''
...
class VNSHardwareInfo(LTMessage):
    ...
    def pack(self):
        # Change '' to b'' (the 'b' makes it a bytes-join)
        return b''.join([intf.pack() for intf in self.interfaces])
